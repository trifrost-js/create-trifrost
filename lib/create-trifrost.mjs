...
import {generateKeys} from './security/index.mjs';
...
const entry = await prompts([
  {
    type: 'select',
    name: 'task',
    message: 'What are we creating today?',
    choices: [
      { title: 'TriFrost Project', value: 'project' },
      { title: 'Security Keys (JWT/Cookie signing, etc)', value: 'security-keys' },
    ],
    initial: 0,
  }
], promptOpts);
...
if (entry.task === 'security-keys') {
    const {algo, out, bits = 2048} = await prompts([
      {
        type: 'select',
        name: 'algo',
        message: 'Which algorithm?',
        choices: [
            { title: 'HS256 (HMAC shared secret)', value: 'hs256' },
            { title: 'HS384 (HMAC shared secret)', value: 'hs384' },
            { title: 'HS512 (HMAC shared secret)', value: 'hs512' },
            { title: 'RS256 (RSA)', value: 'rs256' },
            { title: 'RS384 (RSA)', value: 'rs384' },
            { title: 'RS512 (RSA)', value: 'rs512' },
            { title: 'ES256 (P-256)', value: 'es256' },
            { title: 'ES384 (P-384)', value: 'es384' },
            { title: 'ES512 (P-521)', value: 'es512' },
        ],
      },
      {
        type: prev => prev.startsWith('rs') ? 'select' : null,
        name: 'bits',
        message: 'What key size?',
        choices: [
            { title: '2048 bits (faster, minimum recommended)', value: 2048 },
            { title: '4096 bits (slower, stronger)', value: 4096 },
        ],
        initial: 0,
      },
      {
        type: 'text',
        name: 'out',
        message: 'Where should we output your keys.env?',
        initial: './',
      }
    ], promptOpts);

    const outdir = join(process.cwd(), out);
    generateKeys(algo, outdir, 'keys.env', bits);
    console.log(`\n────────────────────────────────────────────────────────────────────────────────────────────`);
    if (algo.startsWith('hs')) {
        console.log(`\n🧬 HMAC secret written to ${bold(outdir + 'keys.env')}`);
        console.log(`💡 Add to your .env or secret manager: ${bold('SECRET')}`);
        console.log(`👉 Load with ${bold('ctx.env.SECRET')}`);
    } else {
        console.log(`\n🔐 Keypair written to ${bold(outdir + 'keys.env')}`);
        console.log(`💡 Add to your .env or secret manager: ${bold('PRIVATE_KEY')} and ${bold('PUBLIC_KEY')}`);
        console.log(`👉 Load using ${bold('ctx.env.PRIVATE_KEY')} and ${bold('ctx.env.PUBLIC_KEY')}`);
    }
    console.log(`\n────────────────────────────────────────────────────────────────────────────────────────────`);
